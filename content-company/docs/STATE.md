...
> 直近追加: GUI 右側にデバッグ履歴パネル（`/api/debug`・要約＋コピー）、
> アーキ図のフロー線は稼働中の経路だけ流れる（`data-drive` で駆動役割に連動）、
> X/TikTok 下書きの試し読みページ（`/social/preview`）。


- **ブランチ**: `claude/content-sales-company-build-n068kg`
- **場所**: すべて `content-company/` 配下（同リポジトリの `ai-os/` は別物＝汎用実行プレーン）
- **テスト**: `python3 -m unittest discover -s tests` が緑（**53件**）
...
```bash
cd content-company
python3 -m unittest discover -s tests      # まず緑を確認（53件）
