...
> 🧭 **新しいセッションはまず [`docs/STATE.md`](docs/STATE.md) を読む**（現在地・
> 再開手順・不変条件・ハマりどころ・次にやること）。作業の最後に STATE.md を更新。
...
- **標準ライブラリのみ・外部 API 不要**（§36）。`python3 -m unittest discover -s tests` が緑（39件）
